...
def FindEntireNumber(matrix, x_start, y_start):
  ...
  while InBounds(x_first - 1, y_start, xSize,
                 ySize) and matrix[y_start][x_first - 1].isnumeric():
    x_first -= 1

  while InBounds(x_end + 1, y_start, xSize,
                 ySize) and matrix[y_start][x_end + 1].isnumeric():
    x_end += 1
...
def Part1(matrix):
  ...
  for paramaterized_num in adjacent_nums_set:
    engine_sum += NumberNotationToNumber(matrix, paramaterized_num[0],
                                         paramaterized_num[1],
                                         paramaterized_num[2])
