...
def simulateMovement(hLoc, tLoc, direction, steps):
    tailMovement = set()
    for step in range(steps):
        ...
        tailMovement.add((tLoc.x, tLoc.y))
    return tailMovement

def simulateStepHeadTail(hLoc, tLoc, direction, moveHead=False):
    if moveHead:
        # Update hLoc
        if direction == 'U':
            hLoc.y += 1
...
def part1(lines):
    ...
    for line in lines:
        ...
        tailMovement = simulateMovement(hLoc, tLoc, direction, int(steps))
        for item in tailMovement:
            tVisitedLocs.add(item)
    ...
    for line in lines:
        ...
        for step in range(int(steps)):
            for i in range(1,len(knotLocs)):
                head = knotLocs[i-1]
                tail = knotLocs[i]
                simulateStepHeadTail(head, tail, direction, i==1)
                if i == 9:
